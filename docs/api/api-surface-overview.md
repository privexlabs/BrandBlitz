...
| `GET /leaderboard` | `routes/leaderboard.ts` | Global leaderboard |
...
| `GET /leagues` | `routes/leagues.ts` | League listing |
...
| `POST /waitlist` | `routes/waitlist.ts` | Join the waitlist |
...
| `/users/:id/badges` | `routes/users.ts` | Badges for any user (auth required) |
